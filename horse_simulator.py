...
def print_out(rank,type_of_c , total_of_turns):
    ...
    for index, horse in enumerate(rank[:type_of_c]):
        if horse['finish'] is not None:
            ...
            print(f'{index+1:2}. Horse : {horse["horse"]:2}  -> position: {horse["position"]}m → Time: {minutes}m {secondes}s')
        else:
            print(f'{index + 1:2}. Horse : {horse["horse"]:2}  → position: {horse["position"]}m → Time: DNF')
